...
//   // Effect to fetch buildings when facility changes
//   useEffect(() => {
...
//         // Convert facility name to ID - in this case CS1 -> 1, CS2 -> 2
//         const branchId = selectedFacility === "CS1" ? 1 : selectedFacility === "CS2" ? 2 : 0;
        
//         if (branchId > 0) {
//           const buildingsData = await fetchBuildings(branchId);
//           const buildingNames = ["Tất cả", ...buildingsData.map(b => b.building_name)];
...
//   // Reset building selection when facility changes
//   useEffect(() => {
//     setSelectedBuilding("Tất cả");
//   }, [selectedFacility]);
...
//   // --- Handlers ---
//   const handleFacilityChange = async (facility: string) => {
//     setSelectedFacility(facility);
    
//     // Gọi API để lấy danh sách tòa khi thay đổi cơ sở
//     try {
//       if (facility === "Tất cả") {
//         setBuildings(["Tất cả"]);
//         return;
...
//       const branchId = facility === "CS1" ? 1 : facility === "CS2" ? 2 : 0;
//       if (branchId > 0) {
//         const buildingsData = await fetchBuildings(branchId);
//         const buildingNames = ["Tất cả", ...buildingsData.map(b => b.building_name)];
//         setBuildings(buildingNames);
...
function Book() {
  ...
  const handleSearch = () => {
    const filtered = allRoomsData.filter((room) => {
      const facilityMatch =
        selectedFacility === "Tất cả" ||
        room.facility.trim().toUpperCase() === selectedFacility.trim().toUpperCase();

      const buildingMatch =
        selectedBuilding === "Tất cả" ||
        room.details.toUpperCase().trim().startsWith(selectedBuilding.toUpperCase() + "-");

      const roomTypeMatch =
        selectedRoomType === "Tất cả" ||
        room.type.trim().toUpperCase() === selectedRoomType.trim().toUpperCase();
